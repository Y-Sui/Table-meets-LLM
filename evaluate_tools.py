...
def convert_json_to_csv(json_data: list, output_file_name: str):
    ...
    with open(output_file_name, "w", newline=['Choice', 'TabFact', 'HybridQA', 'SQA', 'Feverous', 'ToTTo', 'ToTTo', 'ToTTo', 'ToTTo']) as csv_file:
...
def convert_pandas_to_csv(json_data: list, output_file_name:str):
    ...
    df = pd.DataFrame(columns=['Choice', 'TabFact', 'HybridQA', 'SQA', 'Feverous', 'ToTTo', 'ToTTo', 'ToTTo', 'ToTTo'],
                      index=["NL+Sep", "NL+Sep w/o format explanation", "NL+Sep w/o partition mark", "NL+Sep w/o role prompting",
                             "Markdown", "Markdown w/o format explanation", "Markdown w/o partition mark", "Markdown w/o role prompting",
                             "Json", "Json w/o format explanation", "Json w/o partition mark", "Json w/o role prompting",
                             "XML", "XML w/o format explanation", "XML w/o partition mark", "XML w/o role prompting",
                             "HTML", "HTML w/o format explanation", "HTML w/o partition mark", "HTML w/o role prompting"])
    
    for i in range(len(json_data)):
        for key, value in json_data[i].items():
            _ = key.replace("['", "").replace("']", "").split("_")
            task, input_choices, metric = _[0].title(), format_mapping_func('_'.join(_[1:-1])), _[-1]

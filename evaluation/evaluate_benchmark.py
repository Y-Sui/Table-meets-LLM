...
from main.config import MODELS, DATASETS, TASKS, LINEARIZE
...
class BenchmarkEvaluator:
    def __init__(self, args, line=None):
        ...
        for task in self.tasks:
            ...
            for linearize in self.linearizes:
                mode_dict = {"mode": linearize, "start": [], "end": []}
                ...
                with open(
                    os.path.join(
                        self.experiment_path, txt_file_name, f"unified_{task}.txt"
                    ),
                    "r",
                ) as txt_file:
    ...
    def retrieve_pair(self, task_name: str, model_name: str, mode: dict):
        ...
        ground_truth_path = os.path.join(
            self.experiment_path, txt_file_name, f"unified_{task_name}.jsonl"
        )
        predict_value_path = os.path.join(
            self.experiment_path,
            txt_file_name,
            target_log_file_path,
            "output_dataset",
            f"unified_{task_name}_samples.0.jsonl",
        )
        with open(ground_truth_path, "r") as json_file:
            for idx in range(len(mode["start"])):
                for line in json_file.readlines()[
                    mode["start"][idx] : mode["end"][idx]
                ]:
                    obj = json.loads(line)
                    # dump one sample
    ...
    def save_scores_to_csv(self):
        ...
        df = pd.DataFrame()
        with open(
            f"./output/evaluation/{self.log_file_path}_evaluation.csv", "w", newline=''
        ) as csv_file:
            # score_matrix = np.reshape(self.exact_matching_score, (1, len(self.models), len(list(self.modes.values())[0])))
            score_matrix = np.reshape(
                self.exact_matching_score,
                (len(self.tasks), len(self.models), len(list(self.modes.values())[0])),
            )
                        ...
                        new_df = pd.DataFrame(
                            {
                                "task": [self.tasks[i]],
                                "model": [self.models[j]],
                                "mode": [self.modes[self.tasks[i]][z]['mode']],
                                "score": [score_matrix[i, j, z]],
                            }
                        )
                        df = pd.concat([df, new_df])
        df.to_csv(
            f"./output/evaluation/{self.log_file_path}_evaluation.csv", index=False
        )

    def save_sample_x_to_txt(self):
        with open(
            f"./output/evaluation/{self.log_file_path}_sample.txt",
            'w',
            encoding="utf-8",
        ) as f:
...
def get_arguments():
    ...
    parser.add_argument(
        "--task_group", default="table", type=str, help="Please give the task name"
    )
    parser.add_argument(
        "--log_file_path",
        default="table_benchmarks_20230118_zero_shot_log",
        type=str,
        help="Please indicate the log file path",
    )
    parser.add_argument(
        "--model",
        default=["text003"],
        nargs="+",
        help="Please give the model results you want to evaluate",
    )

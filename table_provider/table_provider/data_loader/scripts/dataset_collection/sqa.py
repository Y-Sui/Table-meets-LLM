...
class MsrSQA(datasets.GeneratorBasedBuilder):
            ...
            features=datasets.Features(
                {
                    ...
                    "table_header": datasets.features.Sequence(
                        datasets.Value("string")
                    ),
                    "table_data": datasets.features.Sequence(
    ...
    def _split_generators(self, dl_manager):
        ...
        data_dir = os.path.join(
            dl_manager.download_and_extract(_URL), "SQA Release 1.0"
        )
        return [
            datasets.SplitGenerator(
                name=datasets.Split.TRAIN,
                gen_kwargs={
                    "filepath": os.path.join(data_dir, "random-split-1-train.tsv"),
                    "data_dir": data_dir,
                },
            ),
            datasets.SplitGenerator(
                ...
                gen_kwargs={
                    "filepath": os.path.join(data_dir, "random-split-1-dev.tsv"),
                    "data_dir": data_dir,
                },
            ),
            datasets.SplitGenerator(
    ...
    def _generate_examples(self, filepath, data_dir):
        ...
        with open(filepath, encoding="utf-8") as f:
            ...
            for idx, item in enumerate(reader):
                ...
                item["answer_coordinates"] = _parse_answer_coordinates(
                    item["answer_coordinates"]
                )
                header, table_data = _load_table_data(
                    os.path.join(data_dir, item["table_file"])
                )
